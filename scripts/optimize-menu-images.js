/**
 * Menu Image Optimization Pipeline
 * Generates AVIF + WebP + JPEG variants at 480/800/1200px from source PNGs.
 * Run: node scripts/optimize-menu-images.js
 */
...
// ── CONFIG ───────────────────────────────────────────────────────────────────
const SRC_DIR  = path.join(process.cwd(), 'menu');              // source masters
const OUT_DIR  = path.join(process.cwd(), 'public', 'images', 'menu'); // delivery output
...
async function main() {
  ...
  // Emit the slug→stem mapping for use in code
  const slugs = files.map(f => path.basename(f, path.extname(f)));
  const mapJson = JSON.stringify(slugs, null, 2);
  const mapPath = path.join(process.cwd(), 'src', 'data', 'menuImageSlugs.json');
  fs.mkdirSync(path.dirname(mapPath), { recursive: true });
  fs.writeFileSync(mapPath, mapJson);
  console.log(`   Slug map      : ${mapPath}`);
}
